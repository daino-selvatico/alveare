...
int main() {
    std::cout << "=== Running GpuBackend (AMD Radeon 890M / Vulkan) Test ===\n";
    GpuBackend gpu;
    assert(gpu.type() == DeviceType::GPU);
    std::cout << "[ok] Device detected: " << gpu.name() << "\n";
    std::cout << "[ok] Vulkan active: " << (gpu.is_vulkan_active() ? "YES" : "NO") << "\n";
    ...
    WeightHandle wh = gpu.create_gemv_weight(N, K, packed.data(), packed.size());
    assert(wh != kInvalidWeight);
    std::cout << "[ok] GPU Weight buffer registered: handle=" << wh << "\n";

    std::vector<bf16> y(N);
    gpu.run_gemv(N, K, wh, x.data(), y.data());
    ...
    std::cout << "[ok] GPU GEMV max parity difference vs reference: " << max_err << "\n";
    assert(max_err < 0.1f);
    ...
    gpu.run_gemm(B, N, K, wh, x_batch.data(), y_batch.data());
    ...
    std::cout << "[ok] GPU GEMM batch parity verified across " << B << " rows.\n";
    ...
    std::cout << "[BENCHMARK] GpuBackend GEMV (" << N << "x" << K << "): "
              << avg_us << " us/call (" << gmacs << " GMAC/s)\n";

    std::cout << "=== ALL GPU BACKEND TESTS PASSED ===\n";
    return 0;
}
