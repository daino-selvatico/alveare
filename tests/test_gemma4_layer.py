...
class TestGemma4Layer(unittest.TestCase):
    
    def test_full_layer(self):
        """
        Verify the complete Gemma-4 decoder layer against CPU reference.
        Also inspects intermediate outputs to locate drift.
        """
        print("\n=== Testing Complete Gemma-4 Decoder Layer ===")
        ref_dir = Path(__file__).resolve().parents[0] / "tools" / "ref" / "data_gemma4"
        if not ref_dir.exists():
            ref_dir = Path(__file__).resolve().parents[1] / "tools" / "ref" / "data_gemma4"
        ...
        k_cache_ref = np.load(ref_dir / "k_cache.npy") # (8, 32, 256)
        v_cache_ref = np.load(ref_dir / "v_cache.npy") # (8, 32, 256)
            
        model.reset_caches()
        model.k_caches[0][:, :pos, :] = k_cache_ref[:, :pos, :].astype(bfloat16)
        model.v_caches[0][:, :pos, :] = v_cache_ref[:, :pos, :].astype(bfloat16)
        
        # Run Gemma layer on NPU
        print("Running Gemma-4 layer on NPU...")
        y_npu = model.run_layer(x_bf16, pos, l=0, use_npu=True)
        print("Gemma-4 layer run completed.")
        ...
        q_actual = model.run_gemv_npu(model.layer_weights[0]["attn_q"], x_norm_ref)[:4096]
        # 3. QK-Norm
        ...
        model.k_caches[0][:, :pos+1, :] = k_cache_ref.astype(bfloat16)
        ...
        attn_proj_actual = model.run_gemv_npu(model.layer_weights[0]["attn_output"], attn_out_ref)[:3840]
        # 7. Gate & Up projections (NPU)
        gate_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_gate"], x_norm2_ref)[:15360]
        up_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_up"], x_norm2_ref)[:15360]
        # 8. Down projection (NPU)
        geglu_out_ref = np.load(ref_dir / "geglu_out.npy").astype(bfloat16)
        down_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_down"], geglu_out_ref)[:3840]
