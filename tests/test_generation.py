...
class TestNPUGeneration(unittest.TestCase):
    ...
    def test_greedy_generation(self):
        ...
        # Wait, inside python, we can just load the model
        ...
        print(f"{'NPU (Alveare)':<40} | {'llama.cpp':<40}")
        print("-" * 83)
        ...
        notes_path = Path(__file__).resolve().parents[1] / "tests" / "generation_test_results.txt"
        ...
        self.assertTrue(any(word in npu_text for word in ["France", "Paris", "capital"]), 
                        f"NPU output {npu_text!r} should contain relevant continuation words for coherence!")
    ...
    def run_llama_cpp_reference(self, prompt_messages, max_new_tokens=20):
        ...
        model_path = "/home/daino/llama-mtp/models/Llama-3.2-1B-Instruct-f16.gguf"
        port = 18080
