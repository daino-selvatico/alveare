...
class TestGemmaLayer(unittest.TestCase):
    
    def test_full_layer(self):
        """
        Verify the complete Gemma-3 decoder layer against Hugging Face reference.
        Also inspects intermediate outputs to locate drift.
        """
        print("\n=== Testing Complete Gemma-3 Decoder Layer ===")
        ref_dir = Path(__file__).resolve().parents[0] / "tools" / "ref" / "data_gemma"
        if not ref_dir.exists():
            ref_dir = Path(__file__).resolve().parents[1] / "tools" / "ref" / "data_gemma"
        ...
        k_cache_ref = np.load(ref_dir / "k_cache.npy") # (1, 32, 256)
        v_cache_ref = np.load(ref_dir / "v_cache.npy") # (32, 1, 256)
        if v_cache_ref.shape == (32, 1, 256):
            v_cache_ref = v_cache_ref.transpose(1, 0, 2)
            
        # Apply RoPE to the reference K cache before loading it into model.k_caches
        # since model.k_caches expects keys with RoPE applied.
        k_cache_rope = np.zeros_like(k_cache_ref)
        for p in range(32):
            k_cache_rope[0, p, :] = model.run_rope_cpu_gemma(k_cache_ref[0, p, :], p, base_freq=10000.0)
            
        model.reset_caches()
        model.k_caches[0][:, :pos, :] = k_cache_rope[:, :pos, :].astype(bfloat16)
        model.v_caches[0][:, :pos, :] = v_cache_ref[:, :pos, :].astype(bfloat16)
        
        # Run Gemma layer on NPU
        print("Running Gemma layer on NPU...")
        y_npu = model.run_layer(x_bf16, pos, l=0, use_npu=True)
        print("Gemma layer run completed.")
        ...
        q_rope_ref = np.load(ref_dir / "q_rope.npy").reshape(-1).astype(bfloat16)
        attn_out_ref = np.load(ref_dir / "attn_out.npy").reshape(-1).astype(bfloat16)
        ...
        # 1. Input RMSNorm
        x_norm_actual = model.run_rmsnorm_cpu(x_bf16, model.layer_attn_norms[0])
        # 2. Query Projection (NPU)
        q_actual = model.run_gemv_npu(model.layer_weights[0]["attn_q"], x_norm_ref)[:1024]
        # 3. QK-Norm
        q_normed_actual = np.zeros_like(q_val_ref)
        for h in range(4):
            q_h = q_val_ref[h * 256 : (h + 1) * 256]
            q_normed_actual[h * 256 : (h + 1) * 256] = model.run_rmsnorm_cpu(q_h, model.layer_q_norms[0])
        # 4. RoPE
        q_rope_actual = model.run_rope_cpu_gemma(q_normed_ref, pos, base_freq=10000.0)
        # 5. Attention (Host CPU)
        model.k_caches[0][:, :pos+1, :] = k_cache_rope.astype(bfloat16)
        model.v_caches[0][:, :pos+1, :] = v_cache_ref.astype(bfloat16)
        attn_out_actual = model.run_attention_host(q_rope_ref, pos, l=0)
        # 6. Output Projection (NPU)
        attn_proj_actual = model.run_gemv_npu(model.layer_weights[0]["attn_output"], attn_out_ref)[:1152]
        # 7. Gate & Up projections (NPU)
        gate_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_gate"], x_norm2_ref)[:6912]
        up_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_up"], x_norm2_ref)[:6912]
        # 8. Down projection (NPU)
        geglu_out_ref = np.load(ref_dir / "geglu_out.npy").astype(bfloat16)
        down_actual = model.run_gemv_npu(model.layer_weights[0]["ffn_down"], geglu_out_ref)[:1152]
        ...
        print("\n--- Sub-op Error Tracking (NPU/Host vs HF Reference) ---")
